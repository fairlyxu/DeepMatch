...
def var_feat_process(data,user_var_feature,item_var_feature,var_features_label_file,embedding_dim=16):
    ...
    for f_name in user_var_feature:
        ...
        user_var_feature_dict[f_name] = {"key2index": key2index, "var_feature_list": var_feature_list,
                                         "max_len": max_len}
        user_varlen_feature_columns.append(
            VarLenSparseFeat(SparseFeat(f_name, vocabulary_size=99999999, embedding_dim=embedding_dim),
                             maxlen=max_len, combiner='mean', length_name=None))

    item_var_feature_dict = {}
    item_varlen_feature_columns = []
    for f_name in item_var_feature:
        key2index, var_feature_list, max_len = get_var_feature(data, f_name)
        item_var_feature_dict[f_name] = {"key2index": key2index, "var_feature_list": var_feature_list,
                                         "max_len": max_len}
...
def feat_process( data,user_var_features,item_var_features,var_features_label_file,
    ...
    user_feature_columns = [SparseFeat(feat, sparse_features_label[feat]["max"] + 1, use_hash=True, embedding_dim=embedding_dim)
                            for i, feat in enumerate(user_sparse_features)] + [DenseFeat(feat, 1, ) for feat in
                                                                               user_dense_features]
    item_feature_columns = [SparseFeat(feat, sparse_features_label[feat]["max"] + 1, embedding_dim=embedding_dim)
